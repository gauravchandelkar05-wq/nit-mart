  ...
  return (
    <div className="inline-flex h-full flex-col gap-5 border-r border-slate-200 sm:min-w-60 bg-white">
      <div className="flex flex-col gap-3 justify-center items-center pt-8 max-sm:hidden">
        <Image
          className="w-14 h-14 rounded-full shadow-sm border border-slate-100"
          src={storeInfo?.logo}
          alt=""
          width={80}
          height={80}
      ...
      <div className="max-sm:mt-6">
        {sidebarLinks.map((link, index) => (
          ...
          >
            ...
            {pathname === link.href && (
              <span className="absolute bg-indigo-600 right-0 top-1.5 bottom-1.5 w-1 sm:w-1.5 rounded-l"></span>
            )}
          </Link>
        ))}
      </div>
