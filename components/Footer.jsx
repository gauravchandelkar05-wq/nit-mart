  ...
  const linkSections = [
    {
      title: "PRODUCTS",
      links: [
        { text: "Books & Study Material", path: "/", icon: null },
        { text: "Project & Lab Kits", path: "/", icon: null },
        { text: "Electronics & Gadgets", path: "/", icon: null },
        { text: "Dorm & Hostel Essentials", path: "/", icon: null },
      ],
    },
    {
      title: "CAMPUS POLICIES",
      links: [
        { text: "FAQ & Safety Guidelines", path: "/faq", icon: null },
        { text: "Terms & Conditions", path: "/faq", icon: null },
        { text: "Cancellation & Returns", path: "/faq", icon: null },
    ...
    },
    {
      ...
      links: [
        // 🔥 FIX 1: tel: protocol
        { text: "+91 7498698455", path: "tel:+917498698455", icon: PhoneIcon },
        // 🔥 FIX 2: mailto: protocol
        {
          text: "nitmart.support@gmail.com",
          path: "mailto:nitmart.support@gmail.com",
          icon: MailIcon,
        },
        // 🔥 FIX 3: Google Maps link with external flag
        {
          text: "Mahurzari",
          path: "https://www.google.com/maps/search/?api=1&query=Mahurzari,Nagpur",
          icon: MapPinIcon,
          external: true,
        },
      ],
    ...
    <footer className="mx-6 bg-white">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-500/30 text-slate-500">
          <div>
            ...
            <p className="max-w-[410px] mt-6 text-sm leading-relaxed">
              Welcome to NIT-Mart, the exclusive marketplace for our campus
              community. From second-hand textbooks and engineering lab kits to
              hostel essentials and electronics, buy and sell what you need
              without ever leaving the college grounds.
            </p>
            <div className="flex items-center gap-3 mt-5">
              {socialIcons.map((item, i) => (
                <Link
                  href={item.link}
                  key={i}
                  target="_blank" // 🔥 FIX 4: Open socials in new tab
                  rel="noopener noreferrer"
                  className="flex items-center justify-center w-10 h-10 bg-slate-100 hover:scale-105 hover:border border-slate-300 transition rounded-full text-slate-500"
                >
                  <item.icon />
                </Link>
              ))}
          ...
          <div className="flex flex-wrap justify-between w-full md:w-[45%] gap-5 text-sm ">
            {linkSections.map((section, index) => (
              <div key={index}>
                <h3 className="font-medium text-slate-700 md:mb-5 mb-3 tracking-wide">
                  {section.title}
                </h3>
                <ul className="space-y-2.5">
                  {section.links.map((link, i) => (
                    <li key={i} className="flex items-center gap-2">
                      {link.icon && <link.icon />}
                      {/* 🔥 FIX 5: Conditional target for external links */}
                      <Link
                        href={link.path}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between py-6 text-sm text-slate-500">
          <p>Copyright 2026 © NIT-Mart. All Rights Reserved.</p>
